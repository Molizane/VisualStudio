...
PX_get_value(pxdoc_t *pxdoc, const char *name, float *value) {
	...
	if(strcmp(name, "numprimkeys") == 0) {
		...
		*value = (float) pxdoc->px_head->px_fileversion/(float)10.0;
		return(0);
...
//			fprintf(stderr, "Blob goes into type 3 block\n");
			...
			for(j=63; j>=0; j--) {
				...
				if(mbbhtab.offset == 0) {
					if(pxblob->seek(pxblob, pxs, -(long)(sizeof(TMbBlockHeader3Table)), SEEK_CUR) < 0) {
						px_error(pxdoc, PX_RuntimeError, _("Could not go to table entry for the blob data."));
						return -1;
					}
					break;
				}
				if(pxblob->seek(pxblob, pxs, -2*(long)(sizeof(TMbBlockHeader3Table)), SEEK_CUR) < 0) {
					px_error(pxdoc, PX_RuntimeError, _("Could not go to table entry for the blob data."));
					return -1;
				}
			}
